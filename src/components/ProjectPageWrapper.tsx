// src/components/ProjectPageWrapper.tsx (Stage2出力形式対応版)
"use client";
...
interface AIResponseStage2Data { // AIの戻り値とFirestoreのstage2形式に合わせる
  keywords: string;
...
export default function ProjectPageWrapper({ projectId, projectName: initialProjectName }: ProjectPageWrapperProps) {
  ...
  const [stage2Data, setStage2Data] = useState<AIResponseStage2Data | null>(null); // Stage 2のStateはAIの戻り値型と一致
    ...
    if (!authLoading && user) {
      const getProjectData = async () => {
        ...
        if (docSnap.exists()) {
          ...
          setStage2Data(data.stage2?.keywords ? data.stage2 as AIResponseStage2Data : null); // Stage 2のデータを読み込み形式修正
          setStage3Data(data.stage3?.catchCopy ? data.stage3 as AIResponseStage3Data : null); // Stage 3のデータを読み込み形式修正
  ...
  const handleAnalyzeStage1to2 = async () => {
    ...
    try {
      ...
      const data = result.data as { success: boolean; stage2Data?: AIResponseStage2Data; keywords?: string; productElements?: ProductElements; customerPersonas?: string; }; // 戻り値の型を調整

      if (data.success && data.keywords && data.productElements && data.customerPersonas) {
        ...
        await updateDoc(projectDocRef, {
          stage1: stage1Data,
          stage2: finalStage2Data, // Stage 2の形式に合わせて保存
          currentStage: 2,
          updatedAt: new Date(),
        });
        setCurrentStage(2);
  ...
  const handleAnalyzeStage2to3 = async () => {
    ...
    try {
      ...
      const result = await generateLpFirstViewFunc(stage2Data); // stage2Dataを直接渡す
      const data = result.data as { success: boolean; stage3Data: AIResponseStage3Data; };
      if (data.success) {
        setStage3Data(data.stage3Data);
        const projectDocRef = doc(db, "users", user!.uid, "projects", projectId);
        await updateDoc(projectDocRef, {
          stage3: data.stage3Data, // Stage 3の形式に合わせて保存
          currentStage: 3,
          updatedAt: new Date(),
        });
        setCurrentStage(3);
